...
function subscribe() {
	...
	//사료급여를 명령
	client.send('feed','1', 0, false)
	document.getElementById("messages").innerHTML += '<span>사료를 급여합니다</span><br/>';
	// 구독 신청하였음을 <div> 영역에 출력
	//document.getElementById("messages").innerHTML += '<span>구독신청: 토픽 ' + topic1 + '</span><br/>';
    //document.getElementById("messages").innerHTML += '<span>구독신청: 토픽 ' + topic2 + '</span><br/>';
	client.subscribe('petStatus'); // 브로커에 구독 신청
    client.subscribe('image_topic'); // 브로커에 구독 신청
	client.subscribe('detections'); // 브로커에 구독 신청
	document.getElementById("messages").innerHTML += '<span>관찰을 시작합니다</span><br/>';
}
...
function unsubscribe() {
	...
	document.getElementById("messages").innerHTML += '<span>관찰중지</span><br/>';
	client.unsubscribe('petStatus', null); // 브로커에 구독 신청 취소
    client.unsubscribe('image_topic', null); // 브로커에 구독 신청 취소
	client.unsubscribe('detections', null); // 브로커에 구독 신청 취소
}
...
function onMessageArrived(msg) { // 매개변수 msg는 도착한 MQTT 메시지를 담고 있는 객체
	...
	if(msg.destinationName=="petStatus"){
        document.getElementById("messages").innerHTML += '<span>' + msg.payloadString + '</span><br/>';
		if(msg.payloadString == "관찰을 끝내고 이미지를 분석합니다.")
		client.unsubscribe('petStatus', null); // 브로커에 구독 신청 취소
	}
	else if(msg.destinationName=="image_topic") {
        var imageSrc = "data:image/jpeg;base64," + msg.payloadString;
        document.getElementById("imageContainer").innerHTML = '<img src="' + imageSrc + '" alt="Pet Image"/>';
		client.unsubscribe('image_topic', null); // 브로커에 구독 신청 취소
    }
	else if(msg.destinationName=="detections"){
		document.getElementById("messages").innerHTML += '<span>' + msg.payloadString + '</span><br/>';
		client.unsubscribe('detections', null); // 브로커에 구독 신청 취소
	}
}
